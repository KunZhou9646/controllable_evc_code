...
tf.app.flags.DEFINE_string('dir_to_features','../emotion_features','Dir to emotion features')
...
def dic2npy(features, feat_dim=513, fs=16000):
 ...
 #   filename = features['filename']
    feats = np.concatenate([sp,f0],axis=1)
...
def extract_and_save_bin_to(dir_to_wav, dir_to_bin, dir_to_features, speakers):
    '''
    ...
    for d in list_dir(dir_to_wav):  # ['Training Set', 'Testing Set']
        ...
        for s in list_dir(path):  # ['SF1', ..., 'TM3']
            ...
            for f in list_full_filenames(path):  # ['10001.wav', ...]
                ...
                features = extract(f)  # [sp, ap, f0, en]
                labels = speakers.index(s) * np.ones(
                    [features.shape[0], 1],
                    np.float32,
                )

                b = os.path.splitext(f)[0] # ['10001', ...]
                _, b = os.path.split(b)


                #emo_path = join(dir_to_features, s)
                #with open(join(emo_path,'{}.npy'.format(b)), 'r') as file_dir:
                #    print(file_dir)
                emo = np.load(dir_to_features + '/' + s + '/' + b + '.npy')
                emo_feats = emo * np.ones(
                    [features.shape[0], 1],
                    np.float32,
                )

                features = np.concatenate([features, emo_feats, labels], 1)
...
def read(
    ...
    with tf.device('cpu'):
        with tf.name_scope('InputSpectralFrame'):
            ...
            #zk
            f0 = tf.cast(value[-259], tf.int64)
             
            #speaker = tf.cast(value[-257:-1],tf.int64)
    ...
    with tf.device('cpu'):
        with tf.name_scope('InputPipline'):
            ...
            return {
                ...
                'f0': value[:, SP_DIM * 2],
                'en': value[:, SP_DIM * 2 + 1],
                'speaker': value[:, SP_DIM * 2 + 1 + 1 : SP_DIM * 2 + 1 +1 +256],
                'filename': key,
            }
