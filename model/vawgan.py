...
class VAWGAN(object):
    ...
    def __init__(self, arch, is_training=False):
        ...
        with tf.name_scope('Generator'):   
            self.y_emb = self._unit_embedding(
                ...
                self.arch['z_dim'],
                'y_embedding')
            self.f_emb = self._unit_embedding(
                self.arch['y_dim'],
                self.arch['f_dim'],
                'f_embedding')
    ...
    def _generator(self, z, y, f, is_training):
        ...
        f = tf.nn.embedding_lookup(self.f_emb, f)

        x = self._merge([z, y, f], subnet['merge_dim'])
        x = lrelu(x)
        with slim.arg_scope(
            [slim.batch_norm],
            scale=True, scope='BN',
    ...
    def loss(self, x_s, y_s,f0_s, x_t, y_t,f0_t):
        def circuit_loop(x, y,f):
            ...
            xh, xh_sig_logit = self._generate(z, y, f, is_training=self.is_training)
        ...
        s = circuit_loop(x_s, y_s, f0_s)
        t = circuit_loop(x_t, y_t, f0_t)
        s2t = circuit_loop(x_s, y_t,f0_t)
    ...
    def decode(self, z, y, f, tanh=False):
        xh, _ = self._generate(z, y, f, is_training=False)
        return xh
